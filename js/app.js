...
angular.module( 'translate' )
	.factory('app_factory', ['$http', function($http) {
		...
		// Save embed code for translation in WP options.
		app_factory.save_embed_code = function(data) {
			var url = 'embed.php';
			data._ajax_nonce = localStorage.getItem( 'translate_nonce' );
			return $http({
				method: 'POST',
				url: url,
				transformRequest: app_factory.trasform_request,
		...
		// Delete embed code for translation in WP options.
		app_factory.sign_out = function(data) {
			var url = 'embed.php';
			data._ajax_nonce = localStorage.getItem( 'translate_nonce' );
			return $http({
				method: 'POST',
...
angular.module( 'translate' )
	.controller('app_controller', ['$scope', '$timeout', '$uibModal', '$log', 'app_factory', function($scope, $timeout, $uibModal, $log, app_factory) {
		...
		$scope.select_language = function($event, lang) {
			...
			if ( ! isSelected) {
				...
				// In case of old user don't let him select laguages more than his current plan.
		...
		$scope.add_new_site = function() {
			...
			var data = {
				...
				details: 'Wordpress plugin is installed.'
			};
			...
			app_factory.add_new_site( data )
				.then(function(res) {
					if (res.data.result) {
						save_embed_code( res.data.website_key );
					} else {
						// Show error message.
						$scope.err_msg = res.data.message;
					}
		...
		var save_embed_code = function(wt_embed_code) {
			var data = { wt_embed_code: wt_embed_code };
			// Save embed_code in localstorage so that we can check if.
			// User is already registered wtih this site.
			localStorage.setItem( 'is_registered', 'yes' );
			// Save embed code in wordpress options.
			app_factory.save_embed_code( data );
			// Show the success screen.
			$scope.step = 5;
		};

		$scope.sign_out = function() {
			// Delete wt_embed_code from options table in database.
			app_factory.sign_out( { delete: true } )
				.then(function(res) {
					$uibModal.open({
...
angular.module( 'translate' )
	.controller('logout_controller', function($scope, $uibModalInstance) {
...
angular.module( 'translate' )
	.controller('plan_controller', function($scope, $uibModalInstance) {
...
angular.module( 'translate' )
	.controller('login_controller', function($scope, $uibModalInstance, $log, app_factory) {
									...
									app_factory.get_embed_code( data )
										.then(function(res) {
											if ( ! res.data.result) {
												...
												$scope.user.embed_code = res.data.website_key;
